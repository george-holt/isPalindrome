...
use crate::{from_bytes, from_string, PalindromeError};
...
fn run_rust_inprocess(parsed: &ParsedCheck, resolved_text: &str) -> Result<bool, PalindromeError> {
    ...
    if parsed.hex_mode {
        ...
        Ok(from_bytes(&data, custom.as_ref()))
    } else {
        from_string(resolved_text, custom.as_ref())
    }
}

fn find_cpp_pal_stdin(repo_root: &Path) -> Option<PathBuf> {
    let root = repo_root.join("cpp");
    [
        root.join("build/pal_stdin.exe"),
        root.join("build/pal_stdin"),
        root.join("build/Release/pal_stdin.exe"),
        root.join("build/Debug/pal_stdin.exe"),
        root.join("out/build/pal_stdin.exe"),
    ]
    .into_iter()
    .find(|c| c.is_file())
}

fn find_c_pal_stdin(repo_root: &Path) -> Option<PathBuf> {
    let root = repo_root.join("c/build");
    [
        root.join("pal_stdin.exe"),
        root.join("pal_stdin"),
        root.join("Release/pal_stdin.exe"),
        root.join("Debug/pal_stdin.exe"),
    ]
    .into_iter()
    .find(|c| c.is_file())
}
...
fn run_thin_backend(
    ...
    match impl_name {
        "nodejs" => match which::which("node") {
            Ok(node) => {
                let script = repo_root.join("nodejs/is-palindrome/pal-cli.mjs");
                if !script.is_file() {
                    return BackendRow {
                        backend: "nodejs".into(),
                        status: "skipped".into(),
                ...
                cmd.arg(&script)
                    .current_dir(repo_root.join("nodejs/is-palindrome"))
                    .stdin(Stdio::piped())
                    .stdout(Stdio::piped())
                    .stderr(Stdio::piped());
        ...
        "cs" => match which::which("dotnet") {
            Ok(dotnet) => {
                let proj = repo_root.join("cs/PalCli/PalCli.csproj");
                if !proj.is_file() {
                    return BackendRow {
                        backend: "cs".into(),
                        status: "skipped".into(),
        ...
        "cpp" => {
            let Some(exe) = find_cpp_pal_stdin(repo_root) else {
                return BackendRow {
                    backend: "cpp".into(),
                    status: "skipped".into(),
                    result: None,
                    message: Some("pal_stdin not found for cpp (build cpp)".into()),
                    code: None,
                    complete: None,
                    reason: None,
                };
        ...
        "c" => {
            let Some(exe) = find_c_pal_stdin(repo_root) else {
                return BackendRow {
                    backend: "c".into(),
                    status: "skipped".into(),
                    result: None,
                    message: Some("pal_stdin not found for c (build c)".into()),
                    code: None,
                    complete: None,
                    reason: None,
                };
...
fn run_py_cli(
    ...
    cmd.current_dir(repo_root)
        .env("PYTHONPATH", repo_root.as_os_str())
        .arg("-m")
        .arg("fixtures.cli")
        .arg("check")
        .arg("--impl")
    ...
    let mut child = match cmd.spawn() {
        ...
        Err(e) => {
            return BackendRow {
                ...
                status: "skipped".into(),
                result: None,
                message: Some(format!("spawn python: {e}")),
                code: None,
                complete: None,
                reason: None,
            };
    ...
    match child.wait_with_output() {
        Ok(out) => {
            ...
            let stdout = String::from_utf8_lossy(&out.stdout);
            let stderr = String::from_utf8_lossy(&out.stderr);
            thin_stdout_to_row("py", &stdout, &stderr, code)
        }
        Err(e) => BackendRow {
            backend: "py".into(),
            status: "incomplete".into(),
...
/// Run one backend; `rest` is argv after `--impl` for py subprocess.
/// `py_stdin` is the stdin body for `--impl py` when stdin mode is used (read once by caller).
pub fn dispatch_backend(
    ...
    match impl_name {
        "rust" => rust_row(parsed, resolved_text),
        "py" => run_py_cli(rest, py_stdin, repo_root),
        "c" | "cpp" | "cs" | "nodejs" => {
...
fn all_rows(
    ...
    BACKENDS_ORDERED
        .iter()
        .map(|b| dispatch_backend(b, parsed, rest, resolved_text, repo_root, py_stdin))
        .collect()
}
...
fn parse_impl_prefix(args: &[String]) -> Result<(String, Vec<String>), i32> {
    if args.len() >= 2 && args[0] == "--impl" {
        ...
        Ok((name, args[2..].to_vec()))
    } else {
        Ok(("rust".to_string(), args.to_vec()))
    }
}
...
pub fn run(args: Vec<String>, repo_root: &Path) -> i32 {
    ...
    let (impl_name, rest) = match parse_impl_prefix(&args) {
        Ok(x) => x,
        Err(c) => return c,
    };
    ...
    let py_stdin = if stdin_needed(&parsed) {
    ...
    if impl_name == "all" {
        let rows = all_rows(&parsed, &rest, &resolved, repo_root, py_stdin);
    ...
    let row = dispatch_backend(&impl_name, &parsed, &rest, &resolved, repo_root, py_stdin);
    if let Some(out_path) = output_json {
