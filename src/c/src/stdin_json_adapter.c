/**
 * Stdin JSON adapter for is_palindrome_cli --impl c (JSON on stdin).
 */
...
int main(void) {
    ...
    if (strcmp(mode, "hex") == 0) {
        ...
        bool r = is_palindrome(data, dlen, custom_buf, custom_n);
        free(data);
        printf("%s\n", r ? "true" : "false");
        rc = r ? 0 : 1;
        ...
        if (st == IS_PALINDROME_UTF8_OK) {
            printf("%s\n", result ? "true" : "false");
            rc = result ? 0 : 1;
        } else if (st == IS_PALINDROME_UTF8_ERR_NON_ASCII) {
