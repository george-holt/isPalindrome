...
public static class Palindrome
{
    ...
    public static bool IsPalindrome(ReadOnlySpan<byte> data, IReadOnlySet<byte>? customDelimiterBytes = null)
    {
        int l = 0;
        int r = data.Length - 1;
        while (true)
    ...
    /// <param name="customDelimiterBytes">Same semantics as byte-mode <see cref="IsPalindrome"/>.</param>
    public static bool IsPalindromeFromUtf8(string text, IReadOnlySet<byte>? customDelimiterBytes = null)
    {
        ...
        if (text.Length == 0)
            return IsPalindrome(ReadOnlySpan<byte>.Empty, customDelimiterBytes);

        var bytes = Encoding.Latin1.GetBytes(text);
        return IsPalindrome(bytes, customDelimiterBytes);
    }
