 ...
 * @returns {boolean}
 */
export function isPalindrome(data, custom) {
  if (data.length === 0) {
    return true;
  }
  let l = 0;
 ...
 * @returns {boolean}
 */
export function isPalindromeFromUtf8(s, custom) {
  for (const c of s) {
  ...
  if (s.length === 0) {
    return isPalindrome(new Uint8Array(0), custom);
  }
  const enc = new TextEncoder();
  return isPalindrome(enc.encode(s), custom);
}
