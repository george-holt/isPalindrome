/**
 * Stdin JSON adapter for is_palindrome_cli --impl nodejs (JSON on stdin).
 * One JSON object on stdin: { "mode": "hex"|"string", "hex"?, "text"?, "custom": number[] }
 */

import { readFileSync } from "node:fs";
import { isPalindrome, isPalindromeFromUtf8, PalindromeError } from "./isPalindrome.js";
...
if (req.mode === "hex") {
  ...
  const r = isPalindrome(out, custom);
...
if (req.mode === "string") {
  ...
  try {
    const r = isPalindromeFromUtf8(text, custom);
    process.stdout.write(r ? "true\n" : "false\n");
    process.exit(r ? 0 : 1);
  } catch (e) {
