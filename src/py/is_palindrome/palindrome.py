...
def is_palindrome(data: bytes, custom_delimiter_bytes: Optional[Set[int]] = None) -> bool:
    l, r = 0, len(data) - 1
    if len(data) == 0:
        return True
    while True:
...
def is_palindrome_from_utf8(
    text: str, custom_delimiter_bytes: Optional[Set[int]] = None
) -> bool:
    for c in text:
        if ord(c) > 0x7F:
    ...
    if len(text) == 0:
        return is_palindrome(b"", custom_delimiter_bytes)
    return is_palindrome(text.encode("latin-1"), custom_delimiter_bytes)
